...
public final class ParBoruvka extends AbstractBoruvka<ParBoruvka.ParComponent> {
    ...
    public void computeBoruvka(final Queue<ParComponent> nodesLoaded,
        ...
        ParComponent curr = null;
        while (!nodesLoaded.isEmpty()) {
            curr = nodesLoaded.poll();
            if (curr == null || !curr.lock.tryLock()) {
                continue;
            }
            ...
            final Edge<ParComponent> e = curr.getMinEdge();
            if (e == null) {
                curr.lock.unlock();
                solution.setSolution(curr);
                break;
            }

            final ParComponent other = e.getOther(curr);
            if (!other.lock.tryLock()) {
                curr.lock.unlock();
                nodesLoaded.add(curr);
                continue;
            }
